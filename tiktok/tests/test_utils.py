...
class TestTikHubAPI(TestCase):
    ...
    def test__request(self, mock_request):
        ...
        mock_response.json.return_value = {
            "data": {"userInfo": {"user": {"uniqueId": "tiktok"}}}
        }
        mock_request.return_value = mock_response

        response = self.tikhub.request(
            "/api/v1/tiktok/web/fetch_user_profile?uniqueId=tiktok"
        )
        response_data = response.json().get("data")

        self.assertEqual(response.status_code, 200, "Should return 200 status code")  # noqa: PT009
        self.assertIsNotNone(response_data, "Should not return empty data")  # noqa: PT009

    @patch("tiktok.utils.requests.request")
    def test_get_user_id(self, mock_request):
        expected_sec_uid = "MS4wLjABAAAAPJwdzPJKzzNfqLlFTCqh4v8_zODCuUEbH4bNCELzegjPXmvN8pirTKmDo4wUzMVl"  # noqa: E501
        mock_response = Mock()
        ...
        self.assertEqual(user_id, expected_sec_uid)  # noqa: PT009
    ...
    def test_get_user_info(self, mock_request):
        ...
        mock_response_user_id.json.return_value = {
            "data": {"userInfo": {"user": {"secUid": expected_sec_uid}}}
        }
        ...
        self.assertEqual(  # noqa: PT009
            user_info["username"], "aangiehsl", "Should return the correct username"
        )
