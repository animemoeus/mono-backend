...
def generate_product_embedding_task(product_id: int, force: bool = True):  # noqa: FBT001, FBT002
    try:
...
@shared_task(autoretry_for=(Exception,), max_retries=3, retry_backoff=True)
def generate_product_embeddings_task(product_ids: list[int], force: bool = True):  # noqa: FBT001, FBT002
    success_count = 0
    fail_count = 0
    ...
    for product_id in product_ids:
        try:
            ...
            fail_count += 1
            logger.exception(
                "Failed generating embedding for product %s in batch task.", product_id
            )
